...
## {{< var planned >}} Read functions

These are the external-facing read functions that are used internally in the CLI commands.
...
/// # Errors
///
/// Returns `ReadMetadataError` if the file cannot be opened (e.g., it doesn't
...
fn read_metadata(path: &Path) -> Result<Package, ReadSourceError> {
    ...
    let file = match file_result {
      Ok(file) => file,
      Err(error) => ReadMetadataError::FileCannotOpen
    }
...
Maybe using [serde-saphyr](https://github.com/bourumir-wyngs/serde-saphyr).
